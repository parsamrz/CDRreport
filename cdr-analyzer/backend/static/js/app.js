...
async function clearDatabase() {
    ...
    const confirmed = confirm('⚠️ هشدار!\n\nآیا از پاک کردن تمام داده‌های دیتابیس اطمینان دارید؟\n\nاین عملیات غیرقابل بازگشت است و تمام تماس‌های ثبت شده حذف خواهند شد.');
    ...
    // Double confirmation
    const doubleConfirmed = confirm('⚠️ تأیید نهایی\n\nآیا واقعاً می‌خواهید تمام داده‌ها را پاک کنید؟');
    
    if (!doubleConfirmed) {
        return;
    }
    
    showUploadStatus('در حال پاک کردن دیتابیس...', 'loading');
    ...
    try {
        ...
        if (!response.ok) {
            const error = await response.json();
            throw new Error(error.detail || 'خطا در پاک کردن دیتابیس');
        }
        ...
        showUploadStatus(`❌ خطا: ${error.message}`, 'error');
    }
}
